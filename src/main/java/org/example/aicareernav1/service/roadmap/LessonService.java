...
public class LessonService {
    ...
    public LessonResponse fillLessonContent(Long lessonId, String userQuery) {
        ...
        context.append(contextCollectorService.getShortContextFromCheckpoint(checkpoint));
        if (context.isEmpty()) {
            context.append("Это только начало обучения... Контекста пока нет...");
        }
        if (checkpoint.getType().equals(CheckpointType.DEEPEN)) {
            Checkpoint parent = checkpoint.getParentCheckpoint();
            context.append(contextCollectorService.getShortContextFromCheckpoint(parent));
        }
        log.info("GENERATION CONTEXT:  {}", context);
        // Делегируем всю сложную логику (RAG, стратегии, LLM) оркестратору
        // Используем название урока в качестве поискового запроса
        theoryOrchestrator.getTheoryForLesson(lessonId, adaptationQuery, config, context.toString());
    ...
    public String collectContext(Checkpoint checkpoint) {
        return contextCollectorService.getShortContextFromCheckpoint(checkpoint);
    }
